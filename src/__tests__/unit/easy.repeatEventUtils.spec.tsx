...
describe('반복 유형 선택 기능', () => {
  ...
  describe('반복 유형 선택', () => {
    it('RS-1: 매일(daily) 선택 → 상태 업데이트', async () => {
      ...
      // Assert: 선택 후 드롭다운을 다시 열었을 때 "매일"이 선택되어 있어야 함
      await user.click(within(repeatTypeSelect).getByRole('combobox'));
      expect(screen.getByRole('option', { name: '매일', selected: true })).toBeInTheDocument();
    });
    ...
    it('RS-2: 매주(weekly) 선택 → 상태 업데이트', async () => {
      ...
      // Assert: 선택 후 드롭다운을 다시 열었을 때 "매주"가 선택되어 있어야 함
      await user.click(within(repeatTypeSelect).getByRole('combobox'));
    ...
    it('RS-3: 매월(monthly) 선택 → 상태 업데이트', async () => {
      ...
      // Assert: 선택 후 드롭다운을 다시 열었을 때 "매월"이 선택되어 있어야 함
      await user.click(within(repeatTypeSelect).getByRole('combobox'));
      expect(screen.getByRole('option', { name: '매월', selected: true })).toBeInTheDocument();
    });
    ...
    it('RS-4: 매년(yearly) 선택 → 상태 업데이트', async () => {
      ...
      // Assert: 선택 후 드롭다운을 다시 열었을 때 "매년"이 선택되어 있어야 함
      await user.click(within(repeatTypeSelect).getByRole('combobox'));
      expect(screen.getByRole('option', { name: '매년', selected: true })).toBeInTheDocument();
    });
  });
